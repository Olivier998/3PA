...
def apply_saps(df: DataFrame, convert_score: bool) -> DataFrame:
    ...
    df_saps['pao2fio2'] = df_saps.apply(lambda x:
                                        transform_pao2fio2(pao2fio2=x['pao2fio2'], cpap=x['cpap'],
                                                           vent=x['vent'], convert_score=convert_score)
                                        , axis=1)
    df_saps = df_saps.drop(columns=['cpap', 'vent'])
...
def transform_pao2fio2(pao2fio2: float, cpap: int, vent: int, convert_score: bool = True) -> int:
    """
    Returns the SAPS-II score for a given patient's pao2/fio2 ratio

    :param pao2fio2: Smallest pao2fio2 value of patient
    :param cpap: 1 if the patient has cpap
    :param vent: 1 if the patient has mechanical ventilation
