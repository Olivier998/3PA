...
class _BaseTreeExporter:
    ...
    def node_to_str(self, tree, node_id, criterion, decision_tree, parent):
        node_values = pd.DataFrame(columns=['tree_train', 'original_th', 'sl_train'])
        ...
        if tree.children_left[node_id] != -1:  # _tree.TREE_LEAF:
            ...
            # original_th
            data_th = original_th[node_id]
            original_th[tree.children_left[node_id]] = data_th[data_th[feature] <= tree.threshold[node_id]]
            original_th[tree.children_right[node_id]] = data_th[data_th[feature] > tree.threshold[node_id]]
            # Testing data
        ...
        perc_th_train = 100.0 * original_th[node_id].shape[0] / original_th[0].shape[0]
        node_values.loc['# patients', 'original_th'] = f"{original_th[node_id].shape[0]} ({round(perc_th_train, 1)})"
        ...
        th_pred_prob = np.around(original_th[node_id]['prediction'].mean(), self.precision)
        th_pred_std = np.around(original_th[node_id]['prediction'].std(), self.precision)
        node_values.loc['prediction', 'original_th'] = f"{th_pred_prob} ({th_pred_std})"
...
def export_graphviz_tree(
    ...
    try:
        ...
        decision_tree.original_th = [None] * decision_tree.tree_.node_count
        decision_tree.original_th[0] = original_th
